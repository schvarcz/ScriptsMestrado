...
*/
int RunFABMAP(cv::FileStorage fs)
{
			   ...
			   bool addNewOnly)
{
	...
	confusion_mat = cv::Scalar(0); // init to 0's
...
void runSeqSLAM(string FirstPath, string SecondPath, string ResultsPath)
{
  vector<Mat> newImages = loadDataset( FirstPath );
  vector<Mat> oldImages = loadDataset( SecondPath );
  ...
  imwrite(ResultsPath+"/enhanced.png",seq_slam.getCorrespondenceMatrix());
  ...
  setprecision(0);
  for( int x = 0; x < newImages.size(); x++ ) {
      ...
      newImages[x].copyTo( Mat(appended, Rect(0, 0, 64, 32) ));

      if( score_ptr[x] < threshold )
        {
          cout << index << "/" << oldImages.size() << ": " << oldImages[index].type() << " - " << appended.type() << endl;
          oldImages[index].copyTo( Mat(appended, Rect(64, 0, 64, 32) ));
        }
...
int help(void)
{
	std::cout << "Usage: FABSeqSLAM -s settingsfile" << std::endl;
	return 0;
}
...
int main(int argc, const char * argv[])
{
  ...
  runSeqSLAM(fs["SeqSLAM"]["FirstPath"], fs["SeqSLAM"]["SecondPath"], fs["SeqSLAM"]["ResultsPath"]);
  //RunFABMAP(fs);

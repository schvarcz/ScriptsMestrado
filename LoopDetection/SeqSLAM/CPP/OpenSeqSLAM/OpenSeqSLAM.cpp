 ...
 * Normalize image
 */
Mat OpenSeqSLAM::normalizeImage( Mat& image ) {
    Mat result = image.clone();
    Mat patch_mean, patch_stddev, temp;

    int patch_area = result.rows * result.cols;

    /* Find the mean and std dev, calc  */
    meanStdDev( result, patch_mean, patch_stddev );
    double mean_val   = patch_mean.at<double>(0, 0);
    double stddev_val = convertToSampleStdDev( patch_stddev.at<double>(0, 0), patch_area );
...
pair<int, double> OpenSeqSLAM::findMatch( Mat& diff_mat, int N, int matching_dist ) {
    ...
    if (N > 60 && N < 70)
    {
        cout << N << ": " << min_index << " - " << min_index + matching_dist / 2 << " - " << diff_mat.rows << " - " << diff_mat.cols << endl;
    }

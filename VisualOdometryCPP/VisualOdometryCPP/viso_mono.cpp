...
vector<double> VisualOdometryMono::estimateMotion (vector<IMatcher::p_match> p_matched) {
    ...
    Matrix Tp,Tc;
    vector<IMatcher::p_match> p_matched_normalized = p_matched;
...
bool VisualOdometryMono::normalizeFeaturePoints(vector<IMatcher::p_match> &p_matched,Matrix &Tp,Matrix &Tc) {

    // shift origins to centroids
    double cpu=0,cpv=0,ccu=0,ccv=0;
    for (vector<IMatcher::p_match>::iterator it = p_matched.begin(); it!=p_matched.end(); it++) {
        cpu += it->u1p;
        cpv += it->v1p;
        ccu += it->u1c;
        ccv += it->v1c;
...
void VisualOdometryMono::fundamentalMatrix (const vector<IMatcher::p_match> &p_matched,const vector<int32_t> &active,Matrix &F) {
    ...
    for (int32_t i=0; i<N; i++) {
        IMatcher::p_match m = p_matched[active[i]];
...
vector<int32_t> VisualOdometryMono::getInlier (vector<IMatcher::p_match> &p_matched,Matrix &F) {
...
void VisualOdometryMono::EtoRt(Matrix &E,Matrix &K,vector<IMatcher::p_match> &p_matched,Matrix &X,Matrix &R,Matrix &t) {
...
int32_t VisualOdometryMono::triangulateChieral (vector<IMatcher::p_match> &p_matched,Matrix &K,Matrix &R,Matrix &t,Matrix &X) {
